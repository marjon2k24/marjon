...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_item'])) {
    ...
    $checkSql = "SELECT inv_id FROM Inventory WHERE item_name = '$item_name' AND class = '$class' AND unit = '$unit'";
    $checkResult = $conn->query($checkSql);

    if ($checkResult->num_rows > 0) {
        $error_message = "Item with this name, class, and unit already exists!";
    } else {
        $sql = "INSERT INTO Inventory (item_name, quantity, class, unit) 
                VALUES ('$item_name', $quantity, '$class', '$unit')"; 
        ...
        <table class="inventory-table">
            ...
            while($row = $result->fetch_assoc()): 
                if ($row['item_name'] != $current_item):
                    ...
                    $countSql = "SELECT COUNT(*) FROM Inventory WHERE item_name = '$current_item'";
                    ...
                    <tr>
                        ...
                        <td class="actions">
                            <form method="post" style="display: inline-block; margin: 0 5px;"> 
                                ...
                                <input type="number" name="new_quantity" value="<?= $row['quantity'] ?>" min="0" required style="width: 50px;">
